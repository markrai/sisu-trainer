...
- **Persistence:** `FitnessState` schema v3 retains formal v1-compatible fields and adds a separate `passiveAerobicObservation` metric with explicit descriptive semantics, source, quality, algorithm, fixed window, raw observed ranges, baseline median, guarded trend, formal anchor, and bounded provenance. The trend percentage is derived from the rounded persisted baseline and guarded-trend watts. The permanent reader enforces the v2 writer's 80–200 BPM, 30–600 W, and fixed-window plausibility bounds. Full rebuild from immutable summaries remains deterministic.
