import {
  ...
  type PersonalizedPrescriptionFallbackReasonV1,
  type PersonalizedPrescriptionFitnessEvidenceSnapshotV1,
  type PersonalizedPrescriptionPhaseEvaluationV1,
  type PersonalizedPrescriptionPolicyV1,
  type PersonalizedPrescriptionProfileSnapshotV1,
  type PersonalizedPrescriptionSafetyChecksV1,
 ...
 */
export const PHASE_E1_SHADOW_POLICY: PersonalizedPrescriptionPolicyV1 = {
...
const SUPPORTED_FITNESS_SCHEMAS = new Set<number>([
  FITNESS_STATE_SCHEMA_VERSION_V1,
  FITNESS_STATE_SCHEMA_VERSION_V2,
  FITNESS_STATE_SCHEMA_VERSION_V3,
]);
const FALLBACK_REASONS = new Set<PersonalizedPrescriptionFallbackReasonV1>([
...
export interface EvaluatePersonalizedPrescriptionInput {
  legacyPrescription: ResolvedWorkoutPrescription;
  workoutIntent: string;
  activity: Activity;
  athleteId: string;
  profile: unknown;
  fitnessState: unknown;
  policy: PersonalizedPrescriptionPolicyV1;
  resolvedAt: string;
}

interface EvidenceResolution {
  profileSnapshot: PersonalizedPrescriptionProfileSnapshotV1 | null;
  fitnessSnapshot: PersonalizedPrescriptionFitnessEvidenceSnapshotV1 | null;
  calibration?: HrWorkloadCalibration;
  fallbackReason?: PersonalizedPrescriptionFallbackReasonV1;
  checks: Pick<
    PersonalizedPrescriptionSafetyChecksV1,
...
function cloneHeartRate(target: ResolvedHeartRateTarget | undefined): ResolvedHeartRateTarget | undefined {
...
function cloneProfileSnapshot(profile: AthleteProfile): PersonalizedPrescriptionProfileSnapshotV1 {
  return {
...
function workloadProvenance(
  calibration: HrWorkloadCalibration
): PersonalizedPrescriptionWorkloadProvenanceV1 | null {
  const sources = [...new Set(calibration.points.map((point) => point.workloadSource))];
  if (sources.length !== 1) return sources.length > 1 ? "mixed" : null;
  return sources[0] === "measured_watts" || sources[0] === "calibrated_at_verified_cadence"
    ? sources[0]
    : null;
}

function cloneFitnessSnapshot(
  state: FitnessState,
  provenance: PersonalizedPrescriptionWorkloadProvenanceV1
): PersonalizedPrescriptionFitnessEvidenceSnapshotV1 | null {
  const metric = state.hrWorkloadCalibration;
  if (!metric || !metric.algorithm || !metric.evidenceSessionIds) return null;
...
function initialChecks(): PersonalizedPrescriptionSafetyChecksV1 {
  return {
...
function copyEvidenceChecks(
  checks: EvidenceResolution["checks"]
): PersonalizedPrescriptionSafetyChecksV1 {
  return { ...initialChecks(), ...checks };
}

function rawCalibrationMetric(value: unknown): Record<string, unknown> | null {
  if (!isObject(value)) return null;
  return isObject(value.hrWorkloadCalibration) ? value.hrWorkloadCalibration : null;
}

function rawCalibrationValue(metric: Record<string, unknown> | null): Record<string, unknown> | null {
  return metric && isObject(metric.value) ? metric.value : null;
}

function resolveEvidence(input: EvaluatePersonalizedPrescriptionInput): EvidenceResolution {
  const checks: EvidenceResolution["checks"] = {
    athleteOwnershipValid: null,
    evidenceSchemaValid: null,
    formalSourceValid: null,
    qualityValid: null,
  ...
  if (!isObject(input.fitnessState) || !SUPPORTED_FITNESS_SCHEMAS.has(input.fitnessState.schemaVersion as number)) {
    checks.athleteOwnershipValid = true;
  ...
  if (
    !isPositiveFinite(calibration.slopeBpmPerWatt) ||
...
function phaseIdentity(
  phase: ResolvedWorkoutPhaseTarget
): Omit<PersonalizedPrescriptionPhaseEvaluationV1, "outcome" | "fallbackReason" | "candidatePower" | "safetyChecks"> {
  return {
...
function fallbackPhase(
  phase: ResolvedWorkoutPhaseTarget,
  reason: PersonalizedPrescriptionFallbackReasonV1,
  checks: PersonalizedPrescriptionSafetyChecksV1
): PersonalizedPrescriptionPhaseEvaluationV1 {
  return { ...phaseIdentity(phase), outcome: "fallback", fallbackReason: reason, safetyChecks: checks };
}

function boundedHeartRate(target: ResolvedHeartRateTarget | undefined): target is { min: number; max: number } {
...
function evaluatePhase(
  ...
  if (input.activity !== "bike") return fallbackPhase(phase, "unsupported_activity", checks);
  ...
  if (!phaseSupported) return fallbackPhase(phase, "unsupported_phase", checks);
  if (
    input.workoutIntent !== "aerobic_base" &&
  ...
  if (evidence.fallbackReason || !evidence.calibration || !evidence.fitnessSnapshot) {
    return fallbackPhase(phase, evidence.fallbackReason ?? "invalid_calibration", checks);
  }
  ...
  if (!checks.candidateValid) return fallbackPhase(phase, "invalid_candidate", checks);
  if (!checks.workloadDomainValid) {
    return fallbackPhase(phase, "outside_observed_workload_range", checks);
  }
...
function policyIsValid(policy: unknown): policy is PersonalizedPrescriptionPolicyV1 {
  ...
  if (
    policy.assessmentExpiryPolicy !== "not_configured" &&
    policy.assessmentExpiryPolicy !== "provisional_characterization"
  ) return false;
  if (policy.assessmentExpiryDays === null && policy.assessmentExpiryPolicy !== "not_configured") return false;
  if (policy.assessmentExpiryDays !== null && policy.assessmentExpiryPolicy !== "provisional_characterization") return false;
...
export function evaluatePersonalizedPrescription(
  ...
  if (!isAthleteId(input.athleteId)) throw new Error("Invalid athlete identity for shadow prescription");
  if (!isIsoTimestamp(input.resolvedAt)) throw new Error("Invalid shadow resolution timestamp");
  if (!policyIsValid(input.policy)) throw new Error("Invalid Phase E1 shadow policy");
...
function parseHeartRate(value: unknown): ResolvedHeartRateTarget | null | undefined {
  ...
  const min = value.min;
  const max = value.max;
  if (min !== undefined && (!isFiniteNumber(min) || min < 0 || min > 250)) return null;
  if (max !== undefined && (!isFiniteNumber(max) || max <= 0 || max > 250)) return null;
...
function parseChecks(value: unknown): PersonalizedPrescriptionSafetyChecksV1 | null {
  if (!isObject(value)) return null;
  const keys: Array<keyof PersonalizedPrescriptionSafetyChecksV1> = [
    "athleteOwnershipValid",
    "evidenceSchemaValid",
    "formalSourceValid",
    "qualityValid",
  ...
  const parsed = {} as PersonalizedPrescriptionSafetyChecksV1;
  for (const key of keys) {
    if (value[key] !== null && typeof value[key] !== "boolean") return null;
    parsed[key] = value[key] as boolean | null;
  }
  return parsed;
}

function parseProfileSnapshot(value: unknown): PersonalizedPrescriptionProfileSnapshotV1 | null | undefined {
  ...
  if (value.ageYears !== undefined && !isPositiveFinite(value.ageYears)) return undefined;
  if (value.bodyMassKg !== undefined && !isPositiveFinite(value.bodyMassKg)) return undefined;
  return {
    profileSchemaVersion: ATHLETE_PROFILE_SCHEMA_VERSION_V1,
    profileUpdatedAt: value.profileUpdatedAt,
    ...(value.ageYears !== undefined ? { ageYears: value.ageYears as number } : {}),
    ...(value.bodyMassKg !== undefined ? { bodyMassKg: value.bodyMassKg as number } : {}),
  };
}

function parseFitnessSnapshot(value: unknown): PersonalizedPrescriptionFitnessEvidenceSnapshotV1 | null | undefined {
  if (value === null) return null;
  if (!isObject(value) || !SUPPORTED_FITNESS_SCHEMAS.has(value.fitnessStateSchemaVersion as number)) return undefined;
  if (!isIsoTimestamp(value.fitnessUpdatedAt) || !isIsoTimestamp(value.metricObservedAt) || !isIsoTimestamp(value.metricUpdatedAt)) {
    return undefined;
  }
  if (Date.parse(value.metricUpdatedAt) < Date.parse(value.metricObservedAt)) return undefined;
  ...
  if (
    !Array.isArray(value.evidenceSessionIds) ||
  ...
  if (!isObject(value.calibration)) return undefined;
  ...
  const points: PersonalizedPrescriptionFitnessEvidenceSnapshotV1["calibration"]["points"] = [];
  ...
  if (!nearlyEqual(points[0].watts, calibration.observedMinWatts as number) ||
      !nearlyEqual(points[points.length - 1].watts, calibration.observedMaxWatts as number)) return undefined;
  if (!isObject(calibration.protocol) || typeof calibration.protocol.id !== "string" || !isPositiveInteger(calibration.protocol.version)) {
    return undefined;
  }
  if (
    calibration.workloadProvenance !== "measured_watts" &&
  ...
  if (calibration.workloadProvenance !== expectedProvenance) return undefined;
  ...
  if (!isObject(calibration.profileInputSnapshot) ||
      ...
      !isPositiveFinite(calibration.profileInputSnapshot.bodyMassKg)) return undefined;
  return {
    fitnessStateSchemaVersion: value.fitnessStateSchemaVersion as 1 | 2 | 3,
    ...
    evidenceSessionIds: [...value.evidenceSessionIds] as string[],
    calibration: {
      slopeBpmPerWatt: calibration.slopeBpmPerWatt,
      interceptBpm: calibration.interceptBpm,
      rSquared: calibration.rSquared,
...
function parsePhase(
  value: unknown,
  evidence: PersonalizedPrescriptionFitnessEvidenceSnapshotV1 | null
): PersonalizedPrescriptionPhaseEvaluationV1 | null {
  if (!isObject(value) || typeof value.phaseId !== "string" || value.phaseId.trim() === "") return null;
  if (value.kind !== "warmup" && value.kind !== "work" && value.kind !== "recovery" && value.kind !== "cooldown") return null;
  const allowedIntensities = new Set([
    "warmup_easy", "aerobic_base", "threshold", "vo2_short", "vo2_long", "recovery", "cooldown", "strength_support",
  ]);
  if (value.intensityId !== undefined && !allowedIntensities.has(value.intensityId as string)) return null;
  if (value.detailName !== undefined && (typeof value.detailName !== "string" || value.detailName.trim() === "")) return null;
  if (value.intervalIndex !== undefined && !isPositiveInteger(value.intervalIndex)) return null;
  if (value.activeStartSec !== undefined && (!isFiniteNumber(value.activeStartSec) || value.activeStartSec < 0)) return null;
  if (value.activeEndSec !== undefined && (!isPositiveFinite(value.activeEndSec) ||
      (isFiniteNumber(value.activeStartSec) && value.activeEndSec <= value.activeStartSec))) return null;
  const activeHeartRate = parseHeartRate(value.activeHeartRate);
  if (activeHeartRate === null) return null;
  const safetyChecks = parseChecks(value.safetyChecks);
  if (!safetyChecks) return null;
  const base = {
    phaseId: value.phaseId,
    kind: value.kind,
    ...(value.intensityId !== undefined ? { intensityId: value.intensityId as PersonalizedPrescriptionPhaseEvaluationV1["intensityId"] } : {}),
  ...
  if (value.outcome === "candidate") {
    if (value.fallbackReason !== undefined || !isObject(value.candidatePower) || !evidence) return null;
    const minWatts = value.candidatePower.minWatts;
    const maxWatts = value.candidatePower.maxWatts;
    if (!isPositiveInteger(minWatts) || !isPositiveInteger(maxWatts) || minWatts > maxWatts) return null;
    if (minWatts < evidence.calibration.observedMinWatts || maxWatts > evidence.calibration.observedMaxWatts) return null;
    if (!boundedHeartRate(activeHeartRate)) return null;
    const observedHrs = evidence.calibration.points.map((point) => point.heartRateBpm);
    if (activeHeartRate.min < Math.min(...observedHrs) || activeHeartRate.max > Math.max(...observedHrs)) return null;
    ...
    if (minWatts !== expectedMin || maxWatts !== expectedMax) return null;
    if (
      safetyChecks.phaseSupported !== true ||
    ...
    return { ...base, candidatePower: { minWatts, maxWatts }, outcome: "candidate" } as PersonalizedPrescriptionPhaseEvaluationV1;
  }
  if (value.outcome !== "fallback" || typeof value.fallbackReason !== "string" ||
      !FALLBACK_REASONS.has(value.fallbackReason as PersonalizedPrescriptionFallbackReasonV1) ||
      value.candidatePower !== undefined) return null;
  return {
    ...base,
    outcome: "fallback",
    fallbackReason: value.fallbackReason as PersonalizedPrescriptionFallbackReasonV1,
  } as PersonalizedPrescriptionPhaseEvaluationV1;
}

/** Strict permanent reader for the Phase E1 shadow schema. */
export function parsePersonalizedPrescriptionEvaluation(
  value: unknown
): PersonalizedPrescriptionEvaluationV1 | null {
  if (!isObject(value) || value.schemaVersion !== PERSONALIZED_PRESCRIPTION_EVALUATION_SCHEMA_VERSION_V1) return null;
  if (!isObject(value.resolver) || value.resolver.id !== PERSONALIZED_PRESCRIPTION_RESOLVER_ID_V1 ||
      value.resolver.version !== PERSONALIZED_PRESCRIPTION_RESOLVER_VERSION_V1) return null;
  if (value.mode !== "shadow" || value.activationEligible !== false) return null;
  if (!isIsoTimestamp(value.resolvedAt) || typeof value.workoutSelector !== "string" || value.workoutSelector.trim() === "" ||
      typeof value.workoutIntent !== "string" || !isAthleteId(value.athleteId)) return null;
  if (value.activity !== "bike" && value.activity !== "elliptical" && value.activity !== "strength") return null;
  const profileInputSnapshot = parseProfileSnapshot(value.profileInputSnapshot);
  if (profileInputSnapshot === undefined) return null;
  const fitnessEvidenceSnapshot = parseFitnessSnapshot(value.fitnessEvidenceSnapshot);
  if (fitnessEvidenceSnapshot === undefined || !policyIsValid(value.policy)) return null;
  if (!Array.isArray(value.phases) || value.phases.length > MAX_PHASES) return null;
  const phases: PersonalizedPrescriptionPhaseEvaluationV1[] = [];
  ...
  if (candidatePhases.length > 0) {
    if (!profileInputSnapshot || !fitnessEvidenceSnapshot ||
        profileInputSnapshot.ageYears === undefined || profileInputSnapshot.bodyMassKg === undefined) return null;
    const evidence = fitnessEvidenceSnapshot;
    if (
      evidence.quality !== "moderate" && evidence.quality !== "high" ||
    ...
    const parsedPolicy = value.policy as unknown as PersonalizedPrescriptionPolicyV1;
    if (parsedPolicy.assessmentExpiryDays !== null) {
      ...
      if (evidenceAgeMs < 0 || evidenceAgeMs > parsedPolicy.assessmentExpiryDays * 24 * 60 * 60 * 1000) return null;
    }
    if (candidatePhases.some((phase) =>
      phase.safetyChecks.athleteOwnershipValid !== true ||
  ...
  return {
    ...
    policy: {
      assessmentExpiryDays: value.policy.assessmentExpiryDays as number | null,
      assessmentExpiryPolicy: value.policy.assessmentExpiryPolicy as PersonalizedPrescriptionPolicyV1["assessmentExpiryPolicy"],
      allowedIntensities: ["aerobic_base", "threshold"],
      roundingRule: "nearest_integer_watt",
      extrapolationPolicy: "none",
    },
