import type { Vo2ProtocolStageEvidence, Vo2ProtocolStageWorkloadEvidence, Vo2WorkloadSource } from "./types.js";
...
export interface BikeTelemetrySample {
  timestamp_sec: number;
  rpm?: number;
  watts?: number;
  /** Observed bike resistance, never the commanded/requested target. */
  observed_resistance?: number;
}
...
export function cadenceInBand(rpm: number, prescribed = VO2_PRESCRIBED_CADENCE_RPM): boolean {
  return Math.abs(rpm - prescribed) <= VO2_CADENCE_TOLERANCE_RPM;
}

function windowBounds(stage: Pick<Vo2ProtocolStageEvidence, "active_start_sec" | "active_end_sec">): {
  start: number;
  end: number;
} {
...
function samplesInWindow(
  samples: readonly BikeTelemetrySample[],
  start: number,
  end: number
): BikeTelemetrySample[] {
  return samples.filter(
    (sample) =>
      Number.isFinite(sample.timestamp_sec) && sample.timestamp_sec >= start && sample.timestamp_sec <= end
  );
}
 ...
 */
export function summarizeVo2StageWorkload(
  stage: Pick<Vo2ProtocolStageEvidence, "active_start_sec" | "active_end_sec" | "calibrated_watts_at_70rpm">,
  samples: readonly BikeTelemetrySample[] = [],
  prescribedCadenceRpm = VO2_PRESCRIBED_CADENCE_RPM
): Vo2ProtocolStageWorkloadEvidence {
  ...
  let source: Vo2WorkloadSource = "prescribed_only";
  let estimatorWatts: number | undefined;
  if (wattsMeasured) {
  ...
  const evidence: Vo2ProtocolStageWorkloadEvidence = {
    source,
  ...
  if (estimatorWatts != null) evidence.estimator_watts = estimatorWatts;
  if (wattsMedian != null) evidence.measured_watts_median = wattsMedian;
  if (rpmMedian != null) evidence.measured_cadence_median_rpm = rpmMedian;
  if (cadenceRatio != null) evidence.cadence_in_band_ratio = cadenceRatio;
  return evidence;
}

export function measuredWorkloadForTests(
  watts: number,
  extras: Partial<Vo2ProtocolStageWorkloadEvidence> = {}
): Vo2ProtocolStageWorkloadEvidence {
  return {
    source: "measured_watts",
...
export function verifiedCadenceWorkloadForTests(
...
export function prescribedOnlyWorkloadForTests(
  calibratedWatts: number,
  extras: Partial<Vo2ProtocolStageWorkloadEvidence> = {}
): Vo2ProtocolStageWorkloadEvidence {
  return {
