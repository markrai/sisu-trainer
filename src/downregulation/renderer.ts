...
(function loadParticlePreferences() {
  try {
    ...
    if (storedMinBpm != null) {
      ...
      if (Number.isFinite(v) && v >= 30 && v < NOISE_ENTROPY_MAX_BPM) noiseEntropyMinBpm = v;
    }
  } catch {
    /* ignore */
  }
...
export function getParticleStyle(): ParticleStyle {
...
export function getNoiseEntropyMinBpm(): number {
  return noiseEntropyMinBpm;
}

export function setNoiseEntropyMinBpm(value: number): void {
...
let starfieldProgram: WebGLProgram | null = null;
let starfieldPositionBuffer: WebGLBuffer | null = null;
let starfieldSeedBuffer: WebGLBuffer | null = null;
let starfieldUTime: WebGLUniformLocation | null = null;
let starfieldUCoherence: WebGLUniformLocation | null = null;
let starfieldUNoiseAmplitude: WebGLUniformLocation | null = null;
let starfieldUGravityStrength: WebGLUniformLocation | null = null;
let starfieldUSpeed: WebGLUniformLocation | null = null;
...
function createStarfieldProgram(gl: WebGL2RenderingContext): WebGLProgram | null {
...
function createStarfieldBuffers(gl: WebGL2RenderingContext): boolean {
...
export function initRenderer(canvas: HTMLCanvasElement): boolean {
  ...
  if (!starfieldProgram || !createStarfieldBuffers(gl)) {
    ...
    if (starfieldProgram) gl.deleteProgram(starfieldProgram);
...
function tick(): void {
  ...
  if (style === "starfield" && starfieldProgram && starfieldPositionBuffer && starfieldSeedBuffer) {
    ...
    if (starfieldUTime) gl.uniform1f(starfieldUTime, time);
    if (starfieldUCoherence) gl.uniform1f(starfieldUCoherence, coherence);
    if (starfieldUNoiseAmplitude) gl.uniform1f(starfieldUNoiseAmplitude, noiseAmplitude);
    if (starfieldUGravityStrength) gl.uniform1f(starfieldUGravityStrength, gravityStrength);
    if (starfieldUSpeed) gl.uniform1f(starfieldUSpeed, starfieldSpeed);
