 ...
 */
export async function generateDownregulationSummary(
  sessionId: string,
  startedAt: number,
  endedAt: number,
  _stats: DownregulationSessionStats
): Promise<WorkoutSummary> {
  const durationMs = endedAt - startedAt;
  const durationMinutes = Math.round(durationMs / (1000 * 60));
  if (durationMinutes > MAX_DURATION_MINUTES) {
    throw new Error(
      `Downregulation duration ${durationMinutes} minutes exceeds maximum of ${MAX_DURATION_MINUTES} minutes. Started: ${new Date(startedAt).toISOString()}, Ended: ${new Date(endedAt).toISOString()}`
    );
  }
  ...
  if (zoneSum !== summary.duration_minutes) {
    ...
    const key = `z${summary.primary_zone}` as keyof typeof summary.zone_minutes;
