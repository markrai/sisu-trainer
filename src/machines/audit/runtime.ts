...
import {
  ...
  type MachineDecisionAuditTimingMode,
...
export interface MachineDecisionAuditState {
  entries: MachineDecisionAuditEntry[];
  lastPhaseId?: string;
  lastWorkPhaseStartId?: string;
  lastEvaluationId?: string;
  deferredDeadlineId?: string;
  lastPhaseDecision?: MachineDecision;
}

export function createMachineDecisionAuditState(): MachineDecisionAuditState {
  return { entries: [] };
}

export function appendMachineDecisionAuditEntries(
  entries: readonly MachineDecisionAuditEntry[],
  extra: readonly MachineDecisionAuditEntry[]
): MachineDecisionAuditEntry[] {
  if (extra.length === 0) return [...entries];
  const next = [...entries, ...extra];
  if (next.length <= MAX_MACHINE_DECISION_AUDIT_ENTRIES) return next;
  return next.slice(next.length - MAX_MACHINE_DECISION_AUDIT_ENTRIES);
}

function workoutClock(workoutElapsedSeconds: number, phaseElapsedSeconds: number, phaseValue: number): number {
  return workoutElapsedSeconds - phaseElapsedSeconds + phaseValue;
}

function timingModeFromFrozen(
  timing: PersonalizedWorkTiming | undefined,
  phaseDurationSeconds: number
): MachineDecisionAuditTimingMode | undefined {
  return timingModeForPersonalizedTiming(timing, workDurationClass(phaseDurationSeconds));
}

function evaluationKindFor(
  observation: WorkEvaluationObservation,
  previousDecision: MachineDecision | undefined
): MachineEvaluationKind {
  if (observation.durationBand === "short") return "short_interval_final";
  if (!previousDecision) return "initial";
  if (previousDecision === "increase") return "after_increase";
  if (previousDecision === "decrease") return "after_decrease";
  return "after_hold";
}

function copyOptionalTargets<T extends { targetHeartRateMin?: number; targetHeartRateMax?: number }>(
  entry: T,
  min?: number,
  max?: number
): T {
  if (min !== undefined) entry.targetHeartRateMin = min;
  if (max !== undefined) entry.targetHeartRateMax = max;
  return entry;
}

function workPhaseStartedEntry(
  observation: WorkPhaseStartObservation,
  workoutElapsedSeconds: number
): MachineWorkPhaseStartedAuditEntry {
  const entry: MachineWorkPhaseStartedAuditEntry = {
    ...
    nextEligibleElapsedSeconds: workoutClock(
      workoutElapsedSeconds,
      observation.phaseElapsedSeconds,
      observation.nextEligiblePhaseElapsedSeconds
    ),
  };
  if (observation.intervalIndex !== undefined) entry.intervalIndex = observation.intervalIndex;
  const timingMode = timingModeFromFrozen(observation.personalizedTiming, observation.phaseDurationSeconds);
  if (timingMode) entry.timingMode = timingMode;
...
function evaluationEntry(
  ...
  workoutElapsedSeconds: number,
  previousDecision: MachineDecision | undefined
): MachineEvaluationAuditEntry | undefined {
  if (
  ...
  const entry: MachineEvaluationAuditEntry = {
    version: MACHINE_DECISION_AUDIT_VERSION,
    kind: "evaluation",
    elapsedSeconds: workoutElapsedSeconds,
    phaseKind: observation.phaseKind,
  ...
  if (observation.constraint !== undefined) entry.constraint = observation.constraint;
  ...
  if (observation.nextEligiblePhaseElapsedSeconds !== undefined) {
    entry.nextEligibleElapsedSeconds = workoutClock(
      workoutElapsedSeconds,
      observation.phaseElapsedSeconds,
      observation.nextEligiblePhaseElapsedSeconds
    );
...
function deferredEntry(
  observation: WorkEvaluationObservation,
  workoutElapsedSeconds: number
): MachineEvaluationDeferredAuditEntry | undefined {
  if (!observation.deferred || observation.eligibleSincePhaseElapsedSeconds === undefined) return undefined;
  const entry: MachineEvaluationDeferredAuditEntry = {
    version: MACHINE_DECISION_AUDIT_VERSION,
    ...
    eligibleSinceElapsedSeconds: workoutClock(
      workoutElapsedSeconds,
      observation.phaseElapsedSeconds,
      observation.eligibleSincePhaseElapsedSeconds
    ),
  };
  if (observation.intervalIndex !== undefined) entry.intervalIndex = observation.intervalIndex;
  return copyOptionalTargets(entry, observation.targetHeartRateMin, observation.targetHeartRateMax);
...
export function observeMachineDecisions(
  state: MachineDecisionAuditState,
  workoutElapsedSeconds: number,
  observations: {
...
): MachineDecisionAuditState {
  let next: MachineDecisionAuditState = {
...
function appendIncoming(
  state: MachineDecisionAuditState,
  incoming: MachineDecisionAuditEntry[]
): MachineDecisionAuditState {
...
function observeWorkPhaseStarted(
  state: MachineDecisionAuditState,
  workoutElapsedSeconds: number,
  observation: WorkPhaseStartObservation
): MachineDecisionAuditState {
  ...
  if (next.lastWorkPhaseStartId === startId) return next;
  return appendIncoming(
    { ...next, lastWorkPhaseStartId: startId },
    [workPhaseStartedEntry(observation, workoutElapsedSeconds)]
  );
}
...
function observeEvaluation(
  state: MachineDecisionAuditState,
  workoutElapsedSeconds: number,
  evaluation: WorkEvaluationObservation
): MachineDecisionAuditState {
  let next = state;
  ...
  if (evaluation.deferred) {
    ...
    if (next.deferredDeadlineId === deadlineId) return next;
  ...
  if (next.lastEvaluationId === evaluationId) return next;
