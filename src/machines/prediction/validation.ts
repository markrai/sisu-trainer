...
import type { MachineShadowResistancePrediction, ShadowResistanceDirection, ShadowValidationStatus } from "./types.js";
...
export interface RealizedShadowValidation {
  signedErrorBpm: number;
  absoluteErrorBpm: number;
  directionMatched: boolean;
}

export interface ShadowDirectionValidation {
  predictionOpportunityCount: number;
...
export function usableShadowSessionId(value: unknown): value is string {
  return typeof value === "string" && value.trim() !== "";
}

export function shadowPredictionEventKey(
  event: Pick<
    MachineShadowResistancePrediction,
    "sessionId" | "direction" | "phaseId" | "changeElapsedSeconds" | "fromResistance" | "actualToResistance"
  >
): string | undefined {
  if (!usableShadowSessionId(event.sessionId)) return undefined;
  return [
    event.sessionId,
    event.direction,
    event.phaseId,
...
function expectedOneLevelDelta(direction: ShadowResistanceDirection): number {
  return direction === "increase" ? 1 : -1;
}

export function isOneLevelValidationOpportunity(
  event: MachineShadowResistancePrediction,
  direction: ShadowResistanceDirection
): boolean {
  if (event.direction !== direction) return false;
  if (!usableShadowSessionId(event.sessionId)) return false;
  if (!isFiniteInteger(event.fromResistance) || !isFiniteInteger(event.actualToResistance)) return false;
  if (event.actualToResistance - event.fromResistance !== expectedOneLevelDelta(direction)) return false;
  return isFiniteInteger(event.predictedHrDeltaForActualStep);
}

export function realizedShadowValidation(
  event: MachineShadowResistancePrediction,
  direction: ShadowResistanceDirection
): RealizedShadowValidation | undefined {
  if (!isOneLevelValidationOpportunity(event, direction)) return undefined;
  if (!isFiniteInteger(event.observedHrDelta)) return undefined;
  const signedErrorBpm = event.observedHrDelta - event.predictedHrDeltaForActualStep;
  const directionMatched =
    direction === "increase" ? event.observedHrDelta > 0 : event.observedHrDelta < 0;
  return {
    signedErrorBpm,
...
function emptyValidation(): ShadowDirectionValidation {
  return {
...
function validationStatus(params: {
  ...
  if (params.highConfidence) return "validated";
  if (
    params.realizedPredictionCount < MIN_SHADOW_VALIDATION_REALIZED_PREDICTIONS ||
...
export function validateShadowDirection(
  ...
  const realized = opportunities
    ...
    .filter((item): item is { event: MachineShadowResistancePrediction; outcome: RealizedShadowValidation } => item !== undefined);
  ...
  const withinToleranceCount = realized.filter(
    (item) => item.outcome.absoluteErrorBpm <= SHADOW_VALIDATION_ERROR_TOLERANCE_BPM
  ).length;
  ...
  const highConfidence =
    realized.length >= MIN_SHADOW_VALIDATION_REALIZED_PREDICTIONS &&
  ...
  const result: ShadowDirectionValidation = {
  ...
  if (directionMatchRate !== undefined) result.directionMatchRate = directionMatchRate;
  if (withinToleranceRate !== undefined) result.withinToleranceRate = withinToleranceRate;
  return result;
}

export function shadowValidationStatusLabel(status: ShadowValidationStatus): string {
  if (status === "validated") return "Strong";
  if (status === "not_validated") return "Needs improvement";
  return "Collecting";
}
