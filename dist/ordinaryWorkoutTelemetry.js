import { ORDINARY_BIKE_TELEMETRY_SCHEMA_VERSION, ORDINARY_BIKE_TELEMETRY_SCHEMA_VERSION_V1, } from "./types.js";
...
function parseControllerResistance(value) {
    if (value === undefined)
        return undefined;
...
function parseObservedMetric(value, source, maxValue) {
    if (value === undefined)
        return undefined;
    if (!isObject(value))
        return null;
    if (!isFiniteNumber(value.value) || value.value < 0 || value.value > maxValue)
        return null;
    if (value.source !== source || !isNonNegativeFinite(value.freshnessMs))
        return null;
...
function parseWatts(value) {
    ...
    if (!isObject(value))
        return null;
    if (!isFiniteNumber(value.value) || value.value < 0 || value.value > 5000)
        return null;
    if (value.source !== "measured_watts" && value.source !== "calibrated_watts")
        return null;
    if (!isNonNegativeFinite(value.freshnessMs))
        return null;
    return { value: value.value, source: value.source, freshnessMs: value.freshnessMs };
}
function parseOrdinaryBikeTelemetryV1(value) {
    ...
    if (value.sourceSampleId !== undefined &&
        (typeof value.sourceSampleId !== "string" || value.sourceSampleId.trim() === "" || value.sourceSampleId.length > 256)) {
        return null;
    }
    if (value.freshnessMs !== undefined && !isNonNegativeFinite(value.freshnessMs))
        return null;
    ...
    if (availability !== "fresh" && (watts || cadenceRpm || observedResistance))
        return null;
    if (availability === "fresh" && (value.sourceSampleId === undefined || value.freshnessMs === undefined))
        return null;
    if (availability === "fresh" && value.freshnessMs > ORDINARY_TELEMETRY_MAX_FRESHNESS_MS)
        return null;
    if (availability === "fresh" &&
        [watts, cadenceRpm, observedResistance].some((metric) => metric !== undefined && metric.freshnessMs !== value.freshnessMs)) {
        return null;
    }
    if (availability === "stale" && ((value.sourceSampleId === undefined) !== (value.freshnessMs === undefined)))
        return null;
    if (availability === "unavailable" && (value.sourceSampleId !== undefined || value.freshnessMs !== undefined))
        return null;
    const sourceSampleId = value.sourceSampleId;
...
export function parseOrdinaryBikeTelemetrySample(value) {
    if (!isObject(value))
        return null;
 ...
 */
export function buildOrdinaryBikeTelemetrySample(input) {
    var _a;
    if (!isOwnerId(input.athleteId) || !isOwnerId(input.sessionId))
        return null;
    if (!isNonNegativeInteger(input.activeSec) || !Number.isFinite(input.observedAtMs))
        return null;
    const observedAt = new Date(input.observedAtMs);
    if (!Number.isFinite(observedAt.getTime()))
        return null;
    const maxFreshnessMs = (_a = input.maxFreshnessMs) !== null && _a !== void 0 ? _a : ORDINARY_TELEMETRY_MAX_FRESHNESS_MS;
    if (!isNonNegativeFinite(maxFreshnessMs))
        return null;
    ...
    const availability = receivedAt == null || !hasSnapshot
        ? "unavailable"
    ...
    if (desiredResistance === null || commandedResistance === null)
        return null;
    const sample = {
    ...
    if (availability !== "fresh" || freshnessMs === undefined || !input.telemetrySnapshotAt)
        return sample;
    sample.sourceSampleId = input.telemetrySnapshotAt;
    sample.freshnessMs = freshnessMs;
    ...
    if (input.observedResistance.current &&
