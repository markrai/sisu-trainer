...
let currentExpectedHeartRate = null;
...
function openCancelModal() {
    ...
    const limitBtn = document.getElementById("vo2LimitReachedButton");
    if (limitBtn) {
    ...
    const endBtn = document.getElementById("endWorkoutConfirmButton");
    if (endBtn)
        endBtn.textContent = vo2EndWorkoutButtonLabel(isVo2);
    const titleEl = document.getElementById("cancelModalTitle");
    if (titleEl)
        titleEl.textContent = vo2CancelModalTitle(isVo2);
    const bodyEl = document.getElementById("cancelModalBody");
    if (bodyEl)
        bodyEl.textContent = vo2CancelModalBody(isVo2);
...
function confirmVo2LimitReached() {
    const btn = document.getElementById("vo2LimitReachedButton");
    if (btn === null || btn === void 0 ? void 0 : btn.disabled)
        return;
    if (btn)
        btn.disabled = true;
    ...
    if (decision.type !== "enter-limit-reached" && btn)
        btn.disabled = false;
}
...
function updateHeartColor(liveBpm, target) {
    const heartIcon = document.getElementById("heartIcon");
    const hrNowEl = document.getElementById("hrNow");
    if (!heartIcon)
        return;
    ...
    let hueRotate = 0;
    if (target.max !== undefined && liveBpm > target.max)
        hueRotate = 270;
    else if (target.min !== undefined && liveBpm < target.min)
        hueRotate = 45;
...
function resolvedPrescriptionForSession(day, blocks, session) {
    var _a, _b, _c;
    if ((_a = session.phasePlan) === null || _a === void 0 ? void 0 : _a.resolvedPrescription)
        return session.phasePlan.resolvedPrescription;
    const rawResolvedAt = Number((_b = session.sessionStart) !== null && _b !== void 0 ? _b : session.startTime);
    return resolveWorkoutPrescription({
        workoutSelector: day,
        blocks,
        hrTargets: session.phasePlan ? session.phasePlan.hrTargets : (_c = getHrTargets()[day]) !== null && _c !== void 0 ? _c : null,
...
function deriveWorkoutState(day, plan, workoutMetadata, base, startTime, paused, pausedElapsed, liveBpm, lastBpmUpdateTime) {
    ...
    if (isVo2WorkoutSelector(day)) {
        ...
        const subsectionName = resolvedPhaseTarget === null || resolvedPhaseTarget === void 0 ? void 0 : resolvedPhaseTarget.detailName;
        if (subsectionName)
...
function recordVo2BikeTelemetryIfActive() {
    ...
    if (state.telemetryStale)
        return;
    const metrics = {};
    if (typeof state.rpm === "number" && Number.isFinite(state.rpm) && state.rpm > 0)
        metrics.rpm = state.rpm;
    if (typeof state.watts === "number" && Number.isFinite(state.watts) && state.watts > 0) {
        metrics.watts = state.watts;
    }
    if (typeof state.observedResistance === "number" &&
...
function renderWorkout(state) {
    ...
    const machineUpdate = session.sessionId && active.activity
        ? updateMachineGuidanceRuntime({
            ...
            heartRateBpm: (_c = active.liveBpm) !== null && _c !== void 0 ? _c : undefined,
...
function renderPersonalizationDiagnostics(model) {
    ...
    if (content)
        content.innerHTML = personalizationDiagnosticsHtml(model);
...
function currentPersonalizationDiagnosticsFilters() {
    const value = (id) => { var _a, _b; return (_b = (_a = document.getElementById(id)) === null || _a === void 0 ? void 0 : _a.value) !== null && _b !== void 0 ? _b : "all"; };
    return {
...
function showWorkoutSummaryModal(summary) {
    ...
    if (vo2El) {
        const view = (summary === null || summary === void 0 ? void 0 : summary.vo2_assessment) ? vo2AssessmentPresentation(summary.vo2_assessment) : null;
        if (view) {
            vo2El.style.display = "";
            vo2El.innerHTML =
...
function registerUiGlobals(phaseBoxEl) {
    ...
    window.confirmVo2LimitReached = confirmVo2LimitReached;
    ...
    window.closePersonalizationDiagnostic = closePersonalizationDiagnostic;
    window.exportPersonalizationDiagnostics = exportPersonalizationDiagnostics;
    ...
    window.presentVo2Assessment = presentVo2Assessment;
    window.closeVo2AssessmentModal = closeVo2AssessmentModal;
